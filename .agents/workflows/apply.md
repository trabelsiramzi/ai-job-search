---
name: apply
description: Run the Drafter-Reviewer workflow for a job posting (URL or text) to evaluate fit and generate tailored CV/cover letter
---
...
Run the full verification checklist from `.agents/rules/job-application-rules.md` now — this is the **only** verification pass in the workflow. Re-read both files once here to verify final state on disk matches your mental model after the Step 4 and Step 5 edits.

### Verification Checklist
Report pass/fail for each item in the .agents/rules/job-application-rules.md verification checklist (factual accuracy, targeting, consistency, quality).
